/* Realtime page for Algorithm 3 (Transformer + PDR + ESKF).
 * Mirrors the trilateration page's proven coordinate transform; adds heading,
 * spray cone, assembly point, and a model (campaign) selector.
...
async function loadModels() {
    const select = document.getElementById('a3-model');
    try {
        const models = await API.get(`/api/training-alg3/maps/${runData.map_id}/models`);
        if (!models.length) {
            select.innerHTML = '<option value="">No campaigns for this map</option>';
            select.disabled = true;
            return;
        }
        select.innerHTML = models.map((m) => {
            const name = m.campaign_name ? `${m.campaign_name} ` : '';
            const label = `${name}(campaign ${m.campaign_id})${m.has_model ? '' : ' — no model'}`;
            return `<option value="${m.campaign_id}" ${m.has_model ? '' : 'disabled'}>${escapeHtml(label)}</option>`;
        }).join('');
        const firstWithModel = models.find((m) => m.has_model);
        if (firstWithModel) select.value = String(firstWithModel.campaign_id);
    } catch (error) {
...
function buildDeviceCards(liveTags) {
    ...
    container.innerHTML = list.map((d) => {
        ...
        const stepRow = d.isAdmin ? '' :
            `<div class="a3-row a3-extra"><span class="a3-key">Last step length (PDR)</span>${f('step')}</div>`;
...
function renderDevicePanel(liveTags) {
    ...
    deviceList(liveTags).forEach((d) => {
        ...
        setField(d.hex, 'step', (live && live.step_length != null) ? `${Number(live.step_length).toFixed(2)} m` : '--');
...
async function pushAdminState() {
    ...
    try {
        await API.post(`/api/training-alg3/${runData.training_run_id}/admin`, {
...
function getStartPayload() {
    const campaignVal = document.getElementById('a3-model').value;
    if (!campaignVal) {
        throw new Error('Please choose a trained Transformer model (campaign).');
    }
    const payload = { campaign_id: parseInt(campaignVal, 10), admin_enabled: adminEnabled };
    if (assemblyPoint) {
...
async function beginRealtime() {
    ...
    await API.post(`/api/training-alg3/${runData.training_run_id}/start`, payload);
...
async function pollState() {
    try {
        const state = await API.get(`/api/training-alg3/${runData.training_run_id}/state`);
        lastState = state;
        renderOverlay(state);
