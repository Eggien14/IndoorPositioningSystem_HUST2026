...
function drawAxes(svg) {
    ...
    svg.appendChild(svgEl('line', { x1: GRID_LEFT, y1: xAxisY, x2: GRID_LEFT + gridWidth + 18, y2: xAxisY, class: 'axis' }));
    svg.appendChild(svgEl('line', { x1: yAxisX, y1: GRID_TOP + gridHeight, x2: yAxisX, y2: GRID_TOP - 18, class: 'axis' }));
...
function drawGrid(svg) {
    ...
    for (let index = 1; index <= appConfig.max_ox * appConfig.max_oy; index += 1) {
        ...
        let cls = 'passable';
        if (blocked.has(index)) cls = 'blocked';
        else if (index === startCell) cls = 'start';

        svg.appendChild(svgEl('rect', { x, y, width: CELL_PX, height: CELL_PX, class: `cell ${cls}` }));
        const label = svgEl('text', { x: x + CELL_PX / 2, y: y + CELL_PX / 2, class: 'cell-index' });
...
function drawStartDot(svg) {
    if (!appConfig.start_center) return;
    const p = coordToSvg(appConfig.start_center.x, appConfig.start_center.y);
    svg.appendChild(svgEl('circle', { cx: p.x, cy: p.y, r: 8, class: 'start-dot' }));
}
...
function drawPredictions(svg) {
    ...
    if (mode === 'points' || mode === 'live') {
        ...
        // path mode: vẫn đánh dấu điểm hiện tại
        const p = coordToSvg(latest.x, latest.y);
        svg.appendChild(svgEl('circle', { cx: p.x, cy: p.y, r: 6, class: 'prediction-dot' }));
    }
}
...
function updateStatus() {
    ...
    document.getElementById('steps-text').textContent = String(state.step_count || 0);

    if (state.latest_prediction) {
        document.getElementById('latest-text').textContent =
            `${state.latest_prediction.x.toFixed(2)}, ${state.latest_prediction.y.toFixed(2)}`;
    } else {
        document.getElementById('latest-text').textContent = '--';
    }
...
async function init() {
    ...
    runtimeState = await api('/api/state');
    document.getElementById('page-title').textContent = `${appConfig.map_name} - ${appConfig.trajectory_name} (PDR)`;
    document.getElementById('trajectory-order').textContent =
        `Default trajectory: ${appConfig.trajectory_cells.join(' -> ')}`;
    document.getElementById('start-text').textContent = String(appConfig.start_cell);
    document.getElementById('model-text').textContent =
        `${appConfig.step_length_model} | offset map ${appConfig.offset_angle}° + bno ${appConfig.offset_angle_bno}°`;
