/* Realtime page for Algorithm 2 (Trilateration: Robust LM, loosely-coupled).
 * Exact clone of the Algorithm 3 page — only the positioning backend differs
 * (UWB ranging instead of RSSI/PDR), so there is no model/campaign picker and the
 * API prefix is /api/training-alg2/. Receive side = uwb_ranging + uwb_id (backend);
...
function buildDeviceCards(liveTags) {
    ...
    container.innerHTML = list.map((d) => {
        ...
        const uwbRows = d.isAdmin ? '' :
            ...
            <div class="a3-row a3-extra"><span class="a3-key">RMS error</span>${f('rms')}</div>`;
...
function renderDevicePanel(liveTags) {
    ...
    deviceList(liveTags).forEach((d) => {
        ...
        setField(d.hex, 'rms', (live && live.rms_error != null) ? `${Number(live.rms_error).toFixed(3)} m` : '--');
...
async function pushAdminState() {
    ...
    try {
        await API.post(`/api/training-alg2/${runData.training_run_id}/admin`, {
...
async function beginRealtime() {
    ...
    await API.post(`/api/training-alg2/${runData.training_run_id}/start`, payload);
...
async function pollState() {
    try {
        const state = await API.get(`/api/training-alg2/${runData.training_run_id}/state`);
        lastState = state;
        renderOverlay(state);
